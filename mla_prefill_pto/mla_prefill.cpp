...
#include "include/mla_pto_cube_ops.h"
...
constexpr int32_t UB_HALF_LINE_SIZE = 128;       // UB_FLOAT_sLINE_SIZE * 2
constexpr int32_t BLOCK_QK = 128;
constexpr int32_t S_BLOCK_STACK = 4;
...
struct LocalTensorView {
    ...
    __aicore__ inline operator __ubuf__ T*() const
    {
...
template <typename T>
__aicore__ __attribute__((always_inline)) inline void gm_to_ub_raw(
    const LocalTensorView<T> &dst, __gm__ T *src, uint32_t n_burst, uint32_t len_burst);

template <typename T>
__aicore__ __attribute__((always_inline)) inline void ub_to_gm_raw(
    __gm__ T *dst, const LocalTensorView<T> &src, uint32_t n_burst, uint32_t len_burst);
...
struct RawAddrTensorView {
    ...
    __aicore__ explicit RawAddrTensorView(uint64_t in) : addr(in) {}
    __aicore__ inline uint64_t GetPhyAddr() const { return addr; }
    __aicore__ inline RawAddrTensorView<T> operator[](uint64_t offset) const
    {
        return RawAddrTensorView<T>(addr + offset * sizeof(T));
    }
    template <typename U>
    __aicore__ inline RawAddrTensorView<U> ReinterpretCast() const
    {
        return RawAddrTensorView<U>(addr);
    }
};
...
public:
    ...
    __aicore__ LocalTensorView<DstDataType> GetBuffer(const uint32_t offset) const
    {
        if constexpr (BufferType_ == BufferType::ASCEND_UB) {
            return LocalTensorView<DstDataType>((uint64_t)reinterpret_cast<__ubuf__ uint8_t *>((uintptr_t)offset));
        } else if constexpr (BufferType_ == BufferType::ASCEND_CB) {
            return LocalTensorView<DstDataType>((uint64_t)reinterpret_cast<__cbuf__ uint8_t *>((uintptr_t)offset));
        } else if constexpr (BufferType_ == BufferType::ASCEND_L0A) {
            return LocalTensorView<DstDataType>((uint64_t)reinterpret_cast<__ca__ uint8_t *>((uintptr_t)offset));
        } else if constexpr (BufferType_ == BufferType::ASCEND_L0B) {
            return LocalTensorView<DstDataType>((uint64_t)reinterpret_cast<__cb__ uint8_t *>((uintptr_t)offset));
        } else if constexpr (BufferType_ == BufferType::ASCEND_L0C) {
            return LocalTensorView<DstDataType>((uint64_t)reinterpret_cast<__cc__ uint8_t *>((uintptr_t)offset));
        } else {
...
struct gm_to_ub_align {
    __aicore__ inline gm_to_ub_align(LocalTensorView<DType> dstTensor, __gm__ DType *srcTensor,
    ...
    {
        (void)leftPaddingNum;
        (void)rightPaddingNum;
        copy_gm_to_ubuf((__ubuf__ DType *)(dstTensor.GetPhyAddr()), srcTensor, sid, nBurst, lenBurst / 32, srcGap / 32, dstGap / 32);
    };
};

template <ArchType ArchTag, typename DType>
struct ub_to_gm_align {
    __aicore__ inline ub_to_gm_align(__gm__ DType *dstTensor, LocalTensorView<DType> srcTensor,
                                     uint8_t sid, uint16_t nBurst, uint32_t lenBurst, uint8_t leftPaddingNum,
                                     uint8_t rightPaddingNum, uint32_t srcGap, uint32_t dstGap)
    {
        (void)leftPaddingNum;
        (void)rightPaddingNum;
        copy_ubuf_to_gm(dstTensor, (__ubuf__ DType *)(srcTensor.GetPhyAddr()), sid, nBurst, lenBurst / 32, srcGap / 32, dstGap / 32);
    };
};
...
__aicore__ __attribute__((always_inline)) inline void gm_to_ub_raw(
    const LocalTensorView<T> &dst, __gm__ T *src, uint32_t n_burst, uint32_t len_burst)
{
    copy_gm_to_ubuf((__ubuf__ T *)(dst.GetPhyAddr()), src, 0, n_burst, len_burst, 0, 0);
}
...
__aicore__ __attribute__((always_inline)) inline void ub_to_gm_raw(
    __gm__ T *dst, const LocalTensorView<T> &src, uint32_t n_burst, uint32_t len_burst)
{
    copy_ubuf_to_gm(dst, (__ubuf__ T *)(src.GetPhyAddr()), 0, n_burst, len_burst, 0, 0);
}
...
template <typename T>
__aicore__ __attribute__((always_inline)) inline void gm_to_l1_nd_nd_raw(__cbuf__ T *dst, __gm__ T *src, uint32_t elem_count)
{
    const uint32_t len_burst = (elem_count * sizeof(T) + 31) / 32;
    copy_gm_to_cbuf(dst, src, 0, 1, len_burst, 0, 0, PAD_NONE);
}

template <typename T>
__aicore__ __attribute__((always_inline)) inline void gm_to_l1_nd_nz_raw(__cbuf__ T *dst, __gm__ T *src, uint32_t n_value,
                                                                       uint32_t d_value, uint32_t src_d_value,
                                                                       uint32_t dst_nz_c0_stride)
{
    copy_gm_to_cbuf_multi_nd2nz_b16(dst, src, 0, 1, n_value, d_value, 0, src_d_value, dst_nz_c0_stride, 1, 0);
}

template <typename T>
__aicore__ __attribute__((always_inline)) inline void l1_to_l0a_vector_raw(
    __ca__ T *dst,
    __cbuf__ T *src,
    uint32_t repeat,
    uint32_t src_stride)
{
    load_cbuf_to_ca(dst, src, 0, repeat, src_stride, 0, 0, 0, inc);
}

template <typename T>
__aicore__ __attribute__((always_inline)) inline void l1_to_l0a_nz_zz_raw(
    __ca__ T *dst,
    __cbuf__ T *src,
    ...
    uint32_t k_part_ceil)
{
    ...
    constexpr uint32_t FRACTAL_SIZE = 512 / sizeof(T);
    constexpr uint32_t COL_BLOCK_SIZE = 32 / sizeof(T);
    for (uint32_t i = 0; i < m_tile_ceil / ROW_BLOCK_SIZE; ++i) {
        load_cbuf_to_ca(
            dst + i * ROW_BLOCK_SIZE * k_part_ceil,
            src + i * FRACTAL_SIZE,
            0,
            static_cast<uint16_t>(k_part_ceil / COL_BLOCK_SIZE),
            m_tile_ceil / ROW_BLOCK_SIZE,
            0,
            0,
            0,
            inc);
...
__aicore__ __attribute__((always_inline)) inline void l1_to_l0b_vector_raw(
    __cb__ T *dst,
    __cbuf__ T *src,
    uint32_t repeat,
    uint32_t src_stride)
{
    if constexpr (TRANSPOSE) {
        load_cbuf_to_cb(dst, src, 0, repeat, src_stride, 0, 0, true, inc);
    } else {
        load_cbuf_to_cb(dst, src, 0, repeat, src_stride, 0, 0, false, inc);
    }
}

template <typename OUT_T, typename IN_T>
__aicore__ __attribute__((always_inline)) inline void l0c_to_gm_nd_raw(
    __gm__ OUT_T *dst,
    __cc__ IN_T *src,
    uint32_t m_size,
    uint32_t n_size,
    ...
    uint32_t dst_stride)
{
    // ND_PARA is required when emitting ND layout from L0C.
    set_nd_para(1);
    if constexpr (std::is_same<OUT_T, half>::value && std::is_same<IN_T, float>::value) {
        copy_matrix_cc_to_gm(
            dst, src, 0, n_size, m_size, dst_stride, src_stride, 0, QuantMode_t::F322F16, 0, 0, 1);
    } else {
        copy_matrix_cc_to_gm(
            dst, src, 0, n_size, m_size, dst_stride, src_stride, 0, QuantMode_t::NoQuant, 0, 0, 1);
    }
}

template<typename IN_DATA_TYPE, typename QKV_DT = IN_DATA_TYPE, typename O_DT = IN_DATA_TYPE, int32_t MASK_TYPE = 0>
class FlashAttentionEncoder {
public:
    __aicore__ __attribute__((always_inline)) inline FlashAttentionEncoder(
        ...
        q_gm(q_gm), q_rope_gm(q_rope_gm), s_gm(s_gm), p_gm(p_gm), o_gm(o_gm), o_tmp_gm(o_tmp_gm), upo_tmp_gm(upo_tmp_gm),
        tiling_para_gm(tiling_para_gm)
    {
        ...
        this->embdv = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 16));
        this->kv_heads = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 4));
        this->total_q_blk_num = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 9));
        this->tiling_head_size = (uint32_t)(*((__gm__ uint32_t *)tiling_para_gm + 10));
        this->tiling_para_size = (uint32_t)(*((__gm__ uint32_t *)tiling_para_gm + 11));
        this->tilingKey = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 12));
        this->max_kv_seqlen = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 14));
        this->window_size = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 18));

        if constexpr(MASK_TYPE == 1) {
            this->is_triu_mask = 1;
        }
    ...
    template <typename DstTensorView, typename SrcTensorView>
    __aicore__ __attribute__((always_inline)) inline void LoadDataToCa(
        DstTensorView dst_tensor, SrcTensorView src_tensor,
        uint32_t round_k, uint32_t qk_round_m, uint32_t qk_m)
    {
        uint32_t round_row = RoundUp<uint32_t>(round_k, 32 / sizeof(QKV_DT));
        if (qk_m == 1) {
            l1_to_l0a_vector_raw<QKV_DT>(
                (__ca__ QKV_DT *)dst_tensor.GetPhyAddr(),
                (__cbuf__ QKV_DT *)src_tensor.GetPhyAddr(),
                NumMatrixsRoundUp<QKV_DT>(round_row),
                1);
        } else {
            l1_to_l0a_nz_zz_raw<QKV_DT>(
                (__ca__ QKV_DT *)dst_tensor.GetPhyAddr(),
                (__cbuf__ QKV_DT *)src_tensor.GetPhyAddr(),
                qk_round_m,
                round_row);
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        bool swa_flag = (window_size > 0 && MASK_TYPE == 2);
        ...
        for (uint32_t process = block_idx; process < process_num; process = next_process) {
            ...
            uint64_t q_offset = addr_q_scalar + head_idx * 128 + (uint64_t)m_idx * pp_m_scalar * q_heads * 128;
            uint64_t k_offset = addr_k_scalar + (head_idx / group_num) * 128;
            uint64_t k_rope_offset = addr_k_scalar / 2 + (head_idx / group_num) * 64;
            uint64_t q_rope_offset = addr_q_scalar / 2 + head_idx * 64 + (uint64_t)m_idx * pp_m_scalar * q_heads * 64;
            if (data_shape_type == 1) {
                q_offset = addr_q_scalar + head_idx * embdv * max_seqlen +(uint64_t) m_idx * pp_m_scalar * stride_qo;//  + 17592186044416 / sizeof(QKV_DT);
                k_offset = addr_k_scalar + (head_idx / group_num) * embdv * max_kv_seqlen;//  + 17592186044416 / sizeof(QKV_DT);
            }

            // Only need load Q once
            if (qk_m == 1) {
                gm_to_l1_nd_nd_raw<QKV_DT>((__cbuf__ QKV_DT *)l1q_buf_addr_tensor.GetPhyAddr(),
                                           ((__gm__ QKV_DT *)q_gm) + q_offset, 128);
                gm_to_l1_nd_nd_raw<QKV_DT>((__cbuf__ QKV_DT *)l1q_buf_addr_tensor[16 * 128].GetPhyAddr(),
                                           ((__gm__ QKV_DT *)q_rope_gm) + q_rope_offset, 64);
            } else {
                gm_to_l1_nd_nz_raw<QKV_DT>((__cbuf__ QKV_DT *)l1q_buf_addr_tensor.GetPhyAddr(),
                                           ((__gm__ QKV_DT *)q_gm) + q_offset, qk_m, 128, stride_qo, qk_round_m);
                gm_to_l1_nd_nz_raw<QKV_DT>((__cbuf__ QKV_DT *)l1q_buf_addr_tensor[qk_round_m * 128].GetPhyAddr(),
                                           ((__gm__ QKV_DT *)q_rope_gm) + q_rope_offset, qk_m, 64, stride_qo / 2,
                                           qk_round_m);
            }
            ...
            if (swa_flag) {
                if (window_size > 0 && window_size < kv_seqlen){
                    ...
                    k_offset += n_start * stride_k * pp_n_scalar;
                    v_offset += n_start * stride_v * pp_n_scalar;
                    k_rope_offset += n_start * stride_k * pp_n_scalar / 2;
                }
            }
            ...
            for (uint32_t n_idx = n_start; n_idx < n_end + launch_delay; n_idx += S_BLOCK_STACK) {
                if (n_idx < n_end) {
                    ...
                    for (uint32_t split_idx = 0; split_idx < S_BLOCK_STACK && n_idx + split_idx < n_end; split_idx++) {
                        ...
                        gm_to_l1_nd_nz_raw<QKV_DT>(
                            (__cbuf__ QKV_DT *)l1k_buf_addr_tensor[kv_pingpong_offset + offset].GetPhyAddr(),
                            ((__gm__ QKV_DT *)k_gm) + k_offset, qk_n, 128, stride_k, qk_round_n);
                        gm_to_l1_nd_nz_raw<QKV_DT>(
                            (__cbuf__ QKV_DT *)l1k_buf_addr_tensor[kv_pingpong_offset + offset + qk_round_n * 128]
                                .GetPhyAddr(),
                            ((__gm__ QKV_DT *)k_rope_gm) + k_rope_offset, qk_n, 64, stride_k / 2, qk_round_n);
                        SET_FLAG(MTE2, MTE1, pingpong_flag);
                        WAIT_FLAG(MTE2, MTE1, pingpong_flag);
                        for (int embed_idx = 0; embed_idx < loopQK; embed_idx++) {
                            ...
                            uint32_t embed_flag = embed_idx % 2;
                            WAIT_FLAG(M, MTE1, embed_flag + 2);
                            l1_to_l0b_vector_raw<QKV_DT, false>(
                                (__cb__ QKV_DT *)l0b_buf_tensor[embed_flag * L0AB_HALF_BUF_SIZE].GetPhyAddr(),
                                (__cbuf__ QKV_DT *)l1k_buf_addr_tensor[kv_pingpong_offset + offset + embed_idx * qk_round_n * 128].GetPhyAddr(),
                                NumMatrixsRoundUp<QKV_DT>(RoundUp<uint32_t>(embedk, 32 / sizeof(QKV_DT)) * qk_round_n),
                                1);
                            ...
                            mla_pto::matmul_hh_f32<128, 256, 128>(
                                l0c_buf_tensor[l0c_offset].GetPhyAddr(),
                                l0a_buf_tensor[embed_idx * L0AB_HALF_BUF_SIZE].GetPhyAddr(),
                                l0b_buf_tensor[embed_flag * L0AB_HALF_BUF_SIZE].GetPhyAddr(), qk_m, qk_n, embedk,
                                initc != 0);
                            PIPE_BARRIER(M);

                            SET_FLAG(M, MTE1, embed_flag + 2);

                            if (last_embed_loop) {
                                SET_FLAG(M, FIX, l0c_pingpong_flag);
                                WAIT_FLAG(M, FIX, l0c_pingpong_flag);

                                l0c_to_gm_nd_raw<half, float>(
                                    (__gm__ half *)(s_gm + ((uint64_t)block_idx * TMP_SIZE +
                                                            (n_idx - n_start) % vect_mod * TMP_SIZE / vect_mod +
                                                            split_idx * pp_n_scalar) * sizeof(half)),
                                    (__cc__ float *)l0c_buf_tensor[l0c_offset].GetPhyAddr(),
                                    qk_m,
                                    qk_round_n,
                                    qk_round_m,
                                    sv_round_n);
                ...
                if (n_idx >= launch_delay + n_start) {
                    ...
                    gm_to_l1_nd_nz_raw<QKV_DT>((__cbuf__ QKV_DT *)l1v_buf_addr_tensor[kv_pingpong_offset].GetPhyAddr(),
                                               ((__gm__ QKV_DT *)v_gm) + v_offset, sv_n, embdv, stride_v, sv_round_n);
                    ...
                    for (uint32_t gm_split_idx = 0; gm_split_idx < m_inner_loop; gm_split_idx++) {
                        ...
                        for (uint32_t l1_k_split_idx = 0; l1_k_split_idx < l1_split_loop; l1_k_split_idx++) {
                            ...
                            if (nowM == 1) {
                                gm_to_l1_nd_nd_raw<QKV_DT>(
                                    (__cbuf__ QKV_DT *)l1p_buf_addr_tensor[l1_offset].GetPhyAddr(),
                                    ((__gm__ QKV_DT *)p_gm) + ((uint64_t)block_idx * TMP_SIZE +
                                    (n_idx - launch_delay - n_start) % vect_mod * TMP_SIZE / vect_mod) *
                                    2 / sizeof(QKV_DT) + l1_k_split_idx * n_slice + gm_p_offset,
                                    RoundUp<uint64_t>(sv_round_n, BlockSize<QKV_DT>()));
                            } else {
                                gm_to_l1_nd_nz_raw<QKV_DT>(
                                    (__cbuf__ QKV_DT *)l1p_buf_addr_tensor[l1_offset].GetPhyAddr(),
                                    ((__gm__ QKV_DT *)p_gm) + ((uint64_t)block_idx * TMP_SIZE +
                                    (n_idx - launch_delay - n_start) % vect_mod * TMP_SIZE / vect_mod) *
                                    2 / sizeof(QKV_DT) + l1_k_split_idx * n_slice + gm_p_offset,
                                    nowM, d, sv_round_n * 2 / sizeof(QKV_DT), nowMRound);
                            }
                            ...
                            for (uint32_t l0_k_split_idx = 0; l0_k_split_idx < d_split_loop; l0_k_split_idx++) {
                                ...
                                for (uint32_t l0b_load_idx = 0; l0b_load_idx < 128 / BLOCK_SIZE; ++l0b_load_idx) {
                                    l1_to_l0b_vector_raw<QKV_DT, true>(
                                        (__cb__ QKV_DT *)l0b_buf_tensor[l0_offset + l0b_load_idx * qk_round_k * BLOCK_SIZE].GetPhyAddr(),
                                        (__cbuf__ QKV_DT *)l1v_buf_addr_tensor[kv_pingpong_offset + l0b_load_idx * CUBE_MATRIX_SIZE +
                                                                               l1_k_split_idx * n_slice * BLOCK_SIZE +
                                                                               l0_k_split_idx * 128 * BLOCK_SIZE].GetPhyAddr(),
                                        qk_round_k / BLOCK_SIZE,
                                        sv_round_n / BLOCK_SIZE);
                                }
                                ...
                                mla_pto::matmul_hh_f32<128, 128, 128>(
                                    l0c_buf_tensor[l0c_offset].GetPhyAddr(),
                                    l0a_buf_tensor[l0_offset].GetPhyAddr(), l0b_buf_tensor[l0_offset].GetPhyAddr(), nowM,
                                    qk_round_k, reduce_d, initC);
                                PIPE_BARRIER(M);
                        ...
                        WAIT_FLAG(M, FIX, l0c_pingpong_flag);
                        // // copy O to gm
                        l0c_to_gm_nd_raw<float, float>(
                            (__gm__ float *)(o_tmp_gm + ((uint64_t)block_idx * TMP_SIZE +
                                                         (n_idx - launch_delay - n_start) % vect_mod * TMP_SIZE / vect_mod +
                                                         gm_split_idx * split_m * qk_round_k) * sizeof(float)),
                            (__cc__ float *)l0c_buf_tensor[l0c_offset].GetPhyAddr(),
                            nowM,
                            qk_round_k,
                            nowMRound,
...
private:
    ...
    using L1View = RawAddrTensorView<QKV_DT>;
    using L0AView = RawAddrTensorView<QKV_DT>;
    using L0BView = RawAddrTensorView<QKV_DT>;
    using L0CView = RawAddrTensorView<float>;

    L1View l1q_buf_addr_tensor{(uint64_t)reinterpret_cast<__cbuf__ QKV_DT *>((uintptr_t)l1q_buf_addr_offset)};
    L1View l1k_buf_addr_tensor{(uint64_t)reinterpret_cast<__cbuf__ QKV_DT *>((uintptr_t)l1k_buf_addr_offset)};
    L1View l1p_buf_addr_tensor{(uint64_t)reinterpret_cast<__cbuf__ QKV_DT *>((uintptr_t)l1p_buf_addr_offset)};
    L1View l1v_buf_addr_tensor{(uint64_t)reinterpret_cast<__cbuf__ QKV_DT *>((uintptr_t)l1v_buf_addr_offset)};

    L0AView l0a_buf_tensor{(uint64_t)reinterpret_cast<__ca__ QKV_DT *>((uintptr_t)0)};
    L0BView l0b_buf_tensor{(uint64_t)reinterpret_cast<__cb__ QKV_DT *>((uintptr_t)0)};
    L0CView l0c_buf_tensor{(uint64_t)reinterpret_cast<__cc__ float *>((uintptr_t)0)};
...
extern "C" __global__ __aicore__ void mla_prefill(
    ...
    __gm__ uint8_t *__restrict__ tiling_para_gm)
{
    uint32_t tilingKey = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 12));
    if (tilingKey == 1) {
        FlashAttentionEncoder<half, half, half, 0> fa_cube(sync, q_gm, q_rope_gm, k_gm, k_rope_gm, v_gm, s_gm, p_gm, o_gm, o_tmp_gm, upo_tmp_gm, tiling_para_gm);
        fa_cube.Run();
    } else if (tilingKey == 2) {
        FlashAttentionEncoder<half, half, half, 1> fa_cube(sync, q_gm, q_rope_gm, k_gm, k_rope_gm, v_gm, s_gm, p_gm, o_gm, o_tmp_gm, upo_tmp_gm, tiling_para_gm);
        fa_cube.Run();
    } else if (tilingKey == 3) {
        FlashAttentionEncoder<half, half, half, 2> fa_cube(sync, q_gm, q_rope_gm, k_gm, k_rope_gm, v_gm, s_gm, p_gm, o_gm, o_tmp_gm, upo_tmp_gm, tiling_para_gm);
        fa_cube.Run();
    }
...
__aicore__ __attribute__((always_inline)) void OnlineSoftmaxStage1FP16(
    const LocalTensorView<S_DTYPE> &s_ub,
    const LocalTensorView<S_DTYPE> &mask_processed_ub,
    const LocalTensorView<S_DTYPE> &local_rowmax_ub,
    const LocalTensorView<S_DTYPE> &hat_rowmax_ub,
    const LocalTensorView<S_DTYPE> &global_rowmax_ub,
    const LocalTensorView<S_DTYPE> &diff16_rowmax_ub,
    const LocalTensorView<EXP_DTYPE> &diff_rowmax_ub,
    const LocalTensorView<EXP_DTYPE> &s_exp_ub,
    const LocalTensorView<EXP_DTYPE> &local_rowsum_ub,
    const LocalTensorView<EXP_DTYPE> &global_rowsum_ub,
    const LocalTensorView<P_DTYPE> &p_ub,
    const LocalTensorView<EXP_DTYPE> &tmp_ub,
    const LocalTensorView<S_DTYPE> &tmp16_ub,
    __gm__ S_DTYPE *s_gm,
    __gm__ P_DTYPE *p_gm,
    bool first_n_iter, half tor,
    uint32_t m, uint32_t n_real, uint32_t n_stride, uint32_t pingpong_flag,
    bool move_flag)
{
    uint32_t round_m = (m + BLOCK_SIZE - 1) / BLOCK_SIZE * BLOCK_SIZE;
    uint32_t round_m_8 = (m + FLOAT_BLOCK_SIZE - 1) / FLOAT_BLOCK_SIZE * FLOAT_BLOCK_SIZE;

    WAIT_FLAG(MTE3, MTE2, pingpong_flag);
    gm_to_ub_raw<S_DTYPE>(s_ub, s_gm, m, n_stride / BLOCK_SIZE);
    ...
    muls_v<ArchType::ASCEND_V220, S_DTYPE>(
        ...
        s_ub,
        (S_DTYPE)tor,
        (m * n_stride + VECTOR_SIZE - 1) / VECTOR_SIZE, // repeat
        1,                                                                // dstBlockStride
        1,                                                                // srcBlockStride
        8,                                                                // dstRepeatStride
        8                                                                 // srcRepeatStride
    ...
    if (n_real == 512) {
        Rowmax<S_DTYPE, RowCalcTile::SPEC_TILE_512>(
            s_ub,
            local_rowmax_ub,
            tmp16_ub,
        ...
        Rowmax<S_DTYPE, RowCalcTile::SPEC_TILE_256>(
            s_ub,
            local_rowmax_ub,
            tmp16_ub,
            round_m, n_real, n_stride
        ...
        Rowmax<S_DTYPE, RowCalcTile::TAIL_TILE>(
            s_ub,
            local_rowmax_ub,
            tmp16_ub,
            round_m, n_real, n_stride
    ...
    if (first_n_iter) {
        // *** hm = lm
        ub_to_ub_raw<S_DTYPE>(hat_rowmax_ub, local_rowmax_ub, round_m / BLOCK_SIZE);
        PIPE_BARRIER(V);
    } else {
        __set_mask(m);
        // *** hm = vmax(lm, gm)
        ...
        PIPE_BARRIER(V);
        // dm fp16 cast to fp32
        conv_v<ArchType::ASCEND_V220, S_DTYPE, EXP_DTYPE>(
            diff_rowmax_ub,
            diff16_rowmax_ub,
            1,  // repeat
            1,                                                                 // dstBlockStride
    ...
    PIPE_BARRIER(V);
    // *** gm = hm
    ub_to_ub_raw<S_DTYPE>(global_rowmax_ub, hat_rowmax_ub, round_m / BLOCK_SIZE);
    PIPE_BARRIER(V);
    // *** hm_block = expand_to_block(hm), 存放于 tv
    brcb_v<ArchType::ASCEND_V220, uint16_t>(
        tmp_ub.template ReinterpretCast<uint16_t>(),
        hat_rowmax_ub.template ReinterpretCast<uint16_t>(),
        1,                             // dstBlockStride
        8,                             // dstRepeatStride
        round_m / FLOAT_BLOCK_SIZE // repeat
    );
    PIPE_BARRIER(V);
    for (uint32_t vsub_idx = 0; vsub_idx < n_real / VECTOR_SIZE; ++vsub_idx) {
        sub_v<ArchType::ASCEND_V220, S_DTYPE>(
            s_ub[vsub_idx * VECTOR_SIZE],
            s_ub[vsub_idx * VECTOR_SIZE],
            tmp_ub.template ReinterpretCast<S_DTYPE>(),
            m,                         // repeat
            1,                             // dstBlockStride
            1,                             // src0BlockStride
            0,                             // src1BlockStride
            n_stride / BLOCK_SIZE, // dstRepeatStride
            n_stride / BLOCK_SIZE, // src0RepeatStride
            1                              // src1RepeatStride
        );
    }
    if (n_real % VECTOR_SIZE > 0) {
        __set_mask(n_real % VECTOR_SIZE);
        sub_v<ArchType::ASCEND_V220, S_DTYPE>(
    ...
    // *** ll = rowsum(ls32)
    if (n_real == 512) {
        Rowsum<EXP_DTYPE, RowCalcTile::SPEC_TILE_512>(
            s_exp_ub,
            local_rowsum_ub,
            tmp_ub,
            round_m_8, n_real, n_stride
        );
    } else if (n_real == 256) {
        Rowsum<EXP_DTYPE, RowCalcTile::SPEC_TILE_256>(
            s_exp_ub,
            local_rowsum_ub,
            tmp_ub,
            round_m_8, n_real, n_stride
        ...
        Rowsum<EXP_DTYPE, RowCalcTile::TAIL_TILE>(
            s_exp_ub,
            local_rowsum_ub,
            tmp_ub,
            round_m_8, n_real, n_stride
    ...
    ub_to_gm_raw<P_DTYPE>(p_gm, p_ub, m, n_stride * 2 / BlockSize<int8_t>());
    SET_FLAG(MTE3, MTE2, pingpong_flag);
    if (first_n_iter) {
        // *** gl = ll
        ub_to_ub_raw<EXP_DTYPE>(global_rowsum_ub, local_rowsum_ub, round_m_8 / FLOAT_BLOCK_SIZE);
        PIPE_BARRIER(V);
    } else {
        __set_mask(m);
        // *** gl = dm * gl
...
template<typename IN_DATA_TYPE, typename QKV_DT = IN_DATA_TYPE, typename O_DT = IN_DATA_TYPE, int32_t MASK_TYPE = 0>
class FlashAttentionEncoderVec {
public:
    __aicore__ __attribute__((always_inline)) inline FlashAttentionEncoderVec(
          ...
          deq_pv_gm(deq_pv_gm), off_pv_gm(off_pv_gm), logN_gm(logN_gm)
    {
        ...
        this->sub_block_idx = get_subblockid();
        this->batch_size = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm));
        this->max_seqlen = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 1));
        this->q_heads = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 2));
        this->embd = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 3));
        ...
        this->tiling_para_size = (uint32_t)(*((__gm__ uint32_t *)tiling_para_gm + 11));
        this->tilingKey = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 12));
        this->long_seq = (uint32_t)(*((__gm__ uint32_t *)tiling_para_gm + 13));
        this->window_size = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 18));
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        for (uint32_t process = block_idx; process < process_num; process = next_process) {
            ...
            uint64_t o_offset = addr_o_scalar + head_idx * embdv + (uint64_t)m_idx * pp_m_scalar * stride_qo;
            if (data_shape_type == 1) {
                o_offset = addr_o_scalar + head_idx * embdv * max_seqlen + (uint64_t)m_idx * pp_m_scalar * stride_qo;
            }
            ...
            for (uint32_t n_idx = n_start; n_idx < n_end + launch_delay; n_idx += S_BLOCK_STACK) {
                if (n_idx < n_end) {
                    ...
                    for (uint32_t m_ind = 0; m_ind < m_end; m_ind++) {
                        ...
                        if constexpr(MASK_TYPE == 1) {
                            if (move_flag) {
                                ...
                                gm_to_ub_align<ArchType::ASCEND_V220, IN_DATA_TYPE>(
                                    mask_ubuf_tensor,
                                    ((__gm__ IN_DATA_TYPE *)mask_gm) + cmp_mask_offset +
                                        (sub_block_idx * qk_m / 2 + m_ind * m_slice) * 512,
                                    0,                       // sid
                                    curr_m,                   // nBurst
                                    qk_n * 2,                // lenBurst
                                    0,                       // leftPaddingNum
                            ...
                            if constexpr(MASK_TYPE == 2) {
                                ...
                                gm_to_ub_align<ArchType::ASCEND_V220, IN_DATA_TYPE>(
                                    mask_ubuf_tensor,
                                    ((__gm__ IN_DATA_TYPE *)mask_gm) + mask_offset +
                                        (sub_block_idx * qk_m / 2 + m_ind * m_slice) * max_seqlen,
                                    0,                       // sid
                                    curr_m,                   // nBurst
                                    qk_n * 2,                // lenBurst
                                    0,                       // leftPaddingNum
                        ...
                        OnlineSoftmaxStage1FP16<half, float, QKV_DT, IN_DATA_TYPE, MASK_TYPE> (
                            ...
                            dm16_ubuf_tensor[(((n_idx - n_start) / S_BLOCK_STACK) % 4) * UB_FLOAT_LINE_SIZE + m_ind * m_slice],
                            dm_ubuf_tensor[(((n_idx - n_start) / S_BLOCK_STACK) % 4) * UB_FLOAT_LINE_SIZE + m_ind * m_slice],
                            ls32_ubuf_tensor,
                            ...
                            tv_ubuf_tensor,
                            tv16_ubuf_tensor,
                            ((__gm__ QKV_DT *)s_gm) + (uint64_t)block_idx * TMP_SIZE + (n_idx - n_start) % vect_mod * TMP_SIZE / vect_mod +
                                    (uint64_t)sub_block_idx * qk_m / 2 * qk_round_n + m_ind * m_slice * qk_round_n,
                            ((__gm__ QKV_DT *)p_gm) + ((uint64_t)block_idx * TMP_SIZE + (n_idx - n_start) % vect_mod * TMP_SIZE / vect_mod +
                                            ((uint64_t)sub_block_idx * qk_m / 2 + m_ind * m_slice) * qk_round_n) * 2 / sizeof(QKV_DT),
                            n_idx == n_start, this->tor,
                            curr_m, qk_n, qk_round_n, pingpong_flag, move_flag
                        );
                ...
                if (n_idx >= launch_delay + n_start) {
                    ...
                    if (n_idx != launch_delay + n_start) {
                        for (uint32_t ms_idx = 0; ms_idx < loop_m; ms_idx++){
                            ...
                            gm_to_ub_raw<float>(
                                lo_ubuf_tensor,
                                ((__gm__ float *)o_tmp_gm) + (uint64_t)block_idx * TMP_SIZE + (n_idx - launch_delay - n_start) % vect_mod * TMP_SIZE / vect_mod +
                                                (uint64_t)sub_block_idx * qk_m / 2 * qk_round_k + resm * qk_round_k,
                                1,
                                nowm * qk_round_k / FLOAT_BLOCK_SIZE);
                        ...
                        gm_to_ub_raw<float>(
                            ...
                            1,
                            sub_m * qk_round_k / FLOAT_BLOCK_SIZE);
                    ...
                    if (n_idx + S_BLOCK_STACK > n_end + launch_delay - 1)  {
                        ...
                        __gm__ IN_DATA_TYPE *o_gm_ptr = (__gm__ IN_DATA_TYPE *)o_gm;
                        ub_to_gm_align<ArchType::ASCEND_V220, IN_DATA_TYPE>(
                            o_gm_ptr + o_offset + (uint64_t)sub_block_idx * qk_m / 2 * stride_qo,
...
private:
    ...
    const uint32_t dm16_ubuf_offset = 8 * UB_UINT8_BLOCK_SIZE + 21 * UB_UINT8_LINE_SIZE;
    const uint32_t lo_ubuf_offset = 9 * UB_UINT8_BLOCK_SIZE;

    AsdopsBuffer<ArchType::ASCEND_V220> buf;
    LocalTensorView<half> ls_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(ls_ubuf_offset);
    LocalTensorView<half> lp_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(lp_ubuf_offset);
    LocalTensorView<float> ls32_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(ls32_ubuf_offset);
    LocalTensorView<half> mask_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(mask_ubuf_offset);
    LocalTensorView<float> lo_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(lo_ubuf_offset);
    LocalTensorView<half> lm_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(lm_ubuf_offset);
    LocalTensorView<half> hm_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(hm_ubuf_offset);
    LocalTensorView<half> gm_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(gm_ubuf_offset);
    LocalTensorView<float> dm_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(dm_ubuf_offset);
    LocalTensorView<float> ll_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(ll_ubuf_offset);
    LocalTensorView<float> gl_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(gl_ubuf_offset);
    LocalTensorView<float> tv_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(tv_ubuf_offset);
    LocalTensorView<half> tv16_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(tv_ubuf_offset);
    LocalTensorView<half> dm16_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, half>(dm16_ubuf_offset);
    LocalTensorView<float> go_ubuf_tensor = buf.GetBuffer<BufferType::ASCEND_UB, float>(go_ubuf_offset);
    uint32_t batch_size{0};
    uint32_t max_seqlen{0};
    uint32_t q_heads{0};
    uint32_t embd{0};
    ...
    __gm__ uint8_t *__restrict__ tiling_para_gm)
{
    uint32_t tilingKey = (uint32_t)(*((__gm__ int32_t *)tiling_para_gm + 12));
    if (tilingKey == 1) {
        FlashAttentionEncoderVec<half, half, half, 0> fa_vec(sync, mask_gm, alibi_coeff_gm, o_gm, s_gm, p_gm, o_tmp_gm,
                                                        tiling_para_gm, deq_qk_gm, off_qk_gm, quant_p_gm, deq_pv_gm,
                                                        off_pv_gm, logN_gm);
        fa_vec.Run();
    } else if (tilingKey == 2) {
        FlashAttentionEncoderVec<half, half, half, 1> fa_vec(sync, mask_gm, alibi_coeff_gm, o_gm, s_gm, p_gm, o_tmp_gm,
                                                        tiling_para_gm, deq_qk_gm, off_qk_gm, quant_p_gm, deq_pv_gm,
                                                        off_pv_gm, logN_gm);
        fa_vec.Run();
    } else if (tilingKey == 3) {
        FlashAttentionEncoderVec<half, half, half, 2> fa_vec(sync, mask_gm, alibi_coeff_gm, o_gm, s_gm, p_gm, o_tmp_gm,
                                                        tiling_para_gm, deq_qk_gm, off_qk_gm, quant_p_gm, deq_pv_gm,
                                                        off_pv_gm, logN_gm);
        fa_vec.Run();
    }
...
// Bisheng host compilation clears __DAV_C220_*; the real mla_prefill bodies above disappear but <<<>>> must
// still resolve. Mirror matmul_cce/matmul.cpp: provide a no-op kernel when neither core macro is set.
    ...
    uint8_t *tiling_para_gm)
{
    ...
    mla_prefill<<<block_dim, nullptr, stream>>>(
        (__gm__ uint8_t *)ffts_addr,
        (__gm__ uint8_t *)q_gm,
        (__gm__ uint8_t *)q_rope_gm,
        (__gm__ uint8_t *)k_gm,
        (__gm__ uint8_t *)k_rope_gm,
        (__gm__ uint8_t *)v_gm,
        (__gm__ uint8_t *)mask_gm,
        (__gm__ uint8_t *)alibi_coeff_gm,
